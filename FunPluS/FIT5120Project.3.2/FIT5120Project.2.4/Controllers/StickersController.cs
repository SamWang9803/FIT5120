...
namespace FIT5120Project.Controllers
{
    public class StickersController : Controller
    {
        ...
        {
            List<Vocabulary> images = db.Vocabularies.ToList();
            return View(images);
        }
        ...
        //update vote number of image
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult set_vote()
        {
            string imagePath = Request["selected_image_path"];
            string imageVote = Request["image_vote"];
            //System.Diagnostics.Debug.WriteLine(imagePath);
            //System.Diagnostics.Debug.WriteLine(imageVote);
            ...
            {
                bool haveItem = false;
                foreach (Vocabulary v in images)
                {
                    if (v.UserId.Equals(imagePath))
                    {
                        v.jokeVocab = imageVote;
                        db.SaveChanges();
                        haveItem = true;
                    }
                }
                ...
                if (!haveItem)
                {
                    Vocabulary voteInfo = new Vocabulary();
                    voteInfo.UserId = imagePath;
                    voteInfo.jokeVocab = imageVote;
                    db.Vocabularies.Add(voteInfo);
                    db.SaveChanges();
                }
            }
            else
            ...
            return RedirectToAction("gallery", "Stickers");
        }
